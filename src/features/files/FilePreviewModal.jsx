            ...
            <div className="relative w-full max-w-2xl bg-white border-4 border-black rounded-3xl overflow-hidden shadow-[0px_0px_0px_4px_rgba(255,255,255,0.2)] animate-in zoom-in-95">
                <div className="h-64 bg-gray-100 border-b-2 border-black flex items-center justify-center relative pattern-dots">
                    {file.type === 'image' ? (
                        <div className="text-center">
                            ...
                            <p className="font-mono text-xs text-gray-400">Image Preview: {file.name}</p>
                        </div>
                    ) : file.type === 'video' ? (
                ...
                <div className="p-8">
                    <div className="flex items-start justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-black mb-1">{file.name}</h2>
                            <p className="text-gray-500 font-medium">Uploaded by <span className="text-black font-bold">{file.user}</span> • {file.time}</p>
                        </div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 border-2 border-gray-100">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div><p className="text-gray-500">File Type</p><p className="font-bold uppercase">{file.type}</p></div>
                            <div><p className="text-gray-500">Size</p><p className="font-bold">{file.size}</p></div>
                        </div>
                    </div>
                </div>
            </div>
