...
import { 
  ...
  Trash2, LogOut, User, Gamepad2, Link
} from 'lucide-react';

export default function App() {
  // --- Global State ---
  const [currentView, setCurrentView] = useState('dashboard'); // 'dashboard', 'chat', 'space-details', 'team'
  ...
  const [isUnityLaunching, setIsUnityLaunching] = useState(false); // Simulates game loading
  ...
  const handleLaunchUnity = () => {
    ...
    setTimeout(() => {
      ...
      // In a real app, this would route to the game container
      ...
      <main className="md:ml-28 p-4 md:p-8 min-h-screen transition-all duration-300">
              ...
              <div className="flex flex-col md:flex-row gap-3 w-full lg:w-auto">
                ...
                <div className="flex bg-white rounded-xl border-2 border-black overflow-hidden shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                  <button 
                    ...
                    className={`p-2.5 hover:bg-gray-100 transition-colors ${viewMode === 'grid' ? 'bg-blue-100 text-blue-600' : 'text-gray-500'}`}
                  >
                    <Grid size={20} />
                  </button>
                  ...
                  <button 
                    ...
                    className={`p-2.5 hover:bg-gray-100 transition-colors ${viewMode === 'list' ? 'bg-blue-100 text-blue-600' : 'text-gray-500'}`}
                  >
                    <List size={20} />
                  </button>
                </div>
                  ...
                  >
                    ...
                    >
                      <div className="absolute top-3 left-3 flex gap-2">
                        {space.isOnline ? (
                          ...
                          <span className="bg-gray-200 text-gray-600 border-2 border-black text-xs font-bold px-2 py-1 rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                            OFFLINE
                          </span>
                        )}
                      </div>
                    ...
                    <div className="p-5 flex-1">
                      ...
                      <div className="flex items-center justify-between border-t-2 border-gray-100 pt-3 mt-auto">
                        ...
                        <button 
                          className="text-sm font-bold text-gray-900 underline decoration-2 decoration-yellow-400 underline-offset-2 hover:decoration-pink-400 transition-all"
                        >
                          Enter
                        </button>
                      </div>
        ...
        {currentView === 'space-details' && activeSpace && (
           <div className="animate-in fade-in slide-in-from-right-8 duration-300">
             <button 
               onClick={() => setCurrentView('dashboard')}
               className="mb-6 flex items-center gap-2 text-gray-500 font-bold hover:text-black hover:-translate-x-1 transition-all"
             >
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               ...
               <div className="lg:col-span-2 space-y-6">
                 ...
                 <div className="bg-white border-2 border-black rounded-2xl p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-wrap gap-4">
                    <button 
                      ...
                      className="flex-1 min-w-[200px] bg-green-400 text-black font-bold py-3 px-4 rounded-xl border-2 border-black hover:bg-green-300 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none transition-all flex items-center justify-center gap-2 group"
                    >
                    ...
                    <button 
                      ...
                      className="flex-1 min-w-[140px] bg-yellow-300 text-black font-bold py-3 px-4 rounded-xl border-2 border-black hover:bg-yellow-200 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none transition-all flex items-center justify-center gap-2"
                    >
                      <MessageSquare size={20}/> Text Chat
                    </button>
                    <button 
                      onClick={handleInvite}
                      className="flex-1 min-w-[140px] bg-white text-black font-bold py-3 px-4 rounded-xl border-2 border-black hover:bg-gray-50 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none transition-all flex items-center justify-center gap-2"
                    >
                      {inviteStatus === 'idle' && <><Link size={20}/> Invite</>}
                      {inviteStatus === 'generating' && <Loader2 size={20} className="animate-spin"/>}
                      {inviteStatus === 'copied' && <><CheckCircle2 size={20} className="text-green-500"/> Copied!</>}
                    </button>
                 </div>
               ...
               <div className="space-y-6">
                  ...
                  <div 
                    ...
                    className="bg-white border-2 border-black rounded-2xl p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] cursor-pointer hover:bg-gray-50 transition-colors group"
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(activeSpace.members || []).slice(0, 6).map((m,i) => (
                        <div key={i} className={`w-10 h-10 rounded-xl border-2 border-black flex items-center justify-center font-bold text-xs ${m.avatar}`}>
                          {m.name[0]}
                        </div>
                      ))}
             ...
             {!activeChatSpace ? (
               <div className="h-full flex flex-col items-center justify-center max-w-4xl mx-auto">
                  <div className="text-center mb-8">
                    <div className="w-20 h-20 bg-yellow-300 border-4 border-black rounded-full flex items-center justify-center mx-auto mb-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                       <MessageSquare size={32} />
                    </div>
                    <h2 className="text-4xl font-black text-gray-900 mb-2">Jump into Chat</h2>
                    <p className="text-gray-500 text-lg font-medium">Choose a space to start chatting with your team.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                    {spaces.map(space => (
                      ...
                      >
                         ...
                         <div className="flex-1">
                           <h3 className="font-bold text-lg group-hover:text-pink-600 transition-colors">{space.name}</h3>
                           <p className="text-xs text-gray-500 font-bold uppercase">{space.type} • {space.userCount} Online</p>
                         </div>
               ...
               <div className="h-full flex gap-6">
                 ...
                 <div className="w-80 bg-white border-2 border-black rounded-3xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden hidden lg:flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                      <div className="p-3 bg-yellow-100 border-2 border-black rounded-xl font-bold flex justify-between items-center cursor-pointer shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                         <span># General</span>
                      </div>
                      <div className="p-3 bg-white border-2 border-transparent hover:border-black rounded-xl font-medium text-gray-600 cursor-pointer hover:bg-gray-50 flex justify-between items-center">
                 ...
                 <div className="flex-1 bg-white border-2 border-black rounded-3xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden flex flex-col relative">
                    ...
                    <div className="p-4 border-b-2 border-black flex justify-between items-center bg-gray-50">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-black text-lg">General</h3>
                          <p className="text-xs font-bold text-gray-500">Topic: Weekly Updates</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px]">
                       <div className="flex items-center justify-center">
                         <span className="bg-gray-200 text-gray-600 text-xs font-bold px-3 py-1 rounded-full border-2 border-black">Today</span>
                       </div>
                       ...
                       {currentMessages.length > 0 ? (
                         currentMessages.map((msg) => (
                           <div key={msg.id} className={`flex gap-4 max-w-[80%] ${msg.isMe ? 'ml-auto flex-row-reverse' : ''} animate-in slide-in-from-bottom-2 duration-300`}>
                              <div className={`w-10 h-10 rounded-full border-2 border-black flex-shrink-0 flex items-center justify-center font-bold text-xs ${msg.avatarColor}`}>
                                {msg.user[0]}
                              </div>
                              <div className={`${msg.isMe ? 'flex flex-col items-end' : ''}`}>
                                <div className={`flex items-baseline gap-2 mb-1 ${msg.isMe ? 'flex-row-reverse' : ''}`}>
                                  ...
                                  <span className="text-xs text-gray-500 font-bold">{msg.time}</span>
                                </div>
                                <div className={`border-2 border-black p-4 shadow-[2px_2px_0px_0px_rgba(0,0,0,0.1)] ${msg.isMe 
                                  ? 'bg-black text-white rounded-tl-2xl rounded-bl-2xl rounded-br-2xl shadow-[4px_4px_0px_0px_rgba(236,72,153,1)]' 
                                  : 'bg-white rounded-tr-2xl rounded-br-2xl rounded-bl-2xl'}`}>
                    ...
                    <div className="p-4 border-t-2 border-black bg-white">
                       <form onSubmit={handleSendMessage} className="relative">
                         <input 
                           ...
                           placeholder={`Message #${activeChatSpace.name}...`}
                         />
                         <button 
                           type="submit"
                           disabled={!chatInput.trim()}
                           className="absolute right-2 top-2 bottom-2 aspect-square bg-yellow-300 border-2 border-black rounded-xl flex items-center justify-center hover:bg-yellow-400 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                         >
                           <Send size={20} />
                         </button>
                       </form>
        ...
        {currentView === 'team' && (
           <div className="flex flex-col items-center justify-center h-[60vh] text-center animate-in zoom-in-95">
              <div className="w-32 h-32 bg-blue-200 rounded-full border-4 border-black flex items-center justify-center mb-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                <Users size={64} />
              </div>
              <h2 className="text-4xl font-black mb-4">Team Directory</h2>
              <p className="text-xl font-medium text-gray-500 max-w-md">Manage your team members, roles, and permissions here. Coming soon!</p>
           </div>
        )}

      </main>

      {/* --- State Debugger (Floating) --- */}
      <div className="fixed bottom-6 right-6 z-50">
         <button 
           onClick={() => setIsDebugOpen(!isDebugOpen)}
           className={`w-14 h-14 rounded-full border-2 border-black flex items-center justify-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all hover:-translate-y-1 ${isDebugOpen ? 'bg-red-500 text-white rotate-12' : 'bg-gray-900 text-white hover:bg-gray-800'}`}
         >
           <Bug size={24} />
         </button>

         {isDebugOpen && (
           <div className="absolute bottom-16 right-0 w-80 bg-black/90 text-green-400 p-4 rounded-2xl border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,0.5)] backdrop-blur-md font-mono text-xs overflow-hidden animate-in slide-in-from-bottom-10">
              <div className="flex justify-between items-center mb-2 border-b border-gray-700 pb-2">
                <span className="font-bold text-white">State Debugger</span>
                <span className="text-[10px] bg-gray-700 px-1 rounded text-white">READ-ONLY</span>
              </div>
              <pre className="overflow-auto max-h-60 custom-scrollbar">
{JSON.stringify({
  currentView,
  activeSpaceId: activeSpace?.id || null,
  membersModal: isMembersModalOpen,
  settingsModal: isSettingsModalOpen
}, null, 2)}
              </pre>
           </div>
         )}
      ...
      {isUnityLaunching && (
         <div className="fixed inset-0 z-[100] bg-black text-white flex flex-col items-center justify-center font-mono">
            ...
            <h2 className="text-4xl font-black animate-pulse">LOADING WORLD...</h2>
            <div className="mt-8 w-64 h-4 border-2 border-white rounded-full overflow-hidden p-1">
               <div className="h-full bg-green-400 animate-[width_2s_ease-out_forwards]" style={{width: '100%'}}></div>
            </div>
         </div>
      )}
         ...
         <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
             ...
             <div className="relative w-full max-w-4xl bg-[#FFFDF5] border-4 border-black rounded-3xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col md:flex-row h-[600px] overflow-hidden animate-in zoom-in-95">
                ...
                <button 
                  ...
                  className="absolute top-4 right-4 z-10 w-10 h-10 bg-white border-2 border-black rounded-full flex items-center justify-center hover:bg-red-100 transition-colors shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none"
                >
                  <X size={20} />
                </button>
                  ...
                  <div className="space-y-4">
                     {activeSpace.members?.map(member => (
                        <div key={member.id} className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border-2 border-transparent hover:border-black transition-all">
                           <div className="flex items-center gap-3">
                              <div className={`w-10 h-10 rounded-full border-2 border-black flex items-center justify-center font-bold ${member.avatar}`}>
                                 {member.name[0]}
                              </div>
                              <div>
                           ...
                           <div className="flex items-center gap-2">
                              ...
                              {member.role !== 'Owner' && (
                                <button className="p-2 text-gray-400 hover:text-red-500 transition-colors" title="Remove User">
                                   <Trash2 size={18} />
                                </button>
                              )}
                           </div>
             ...
             <div className="flex-1 overflow-y-auto p-6 bg-[url('https://www.transparenttextures.com/patterns/grid-me.png')]">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                  ...
                  >
                     {uploadState === 'idle' ? (
                       <>
                         <div className="w-12 h-12 bg-white rounded-full border-2 border-gray-300 flex items-center justify-center mb-3 group-hover:border-blue-500 group-hover:scale-110 transition-all">
                            <UploadCloud size={24} />
                         </div>
                       ...
                       <div className="w-full">
                          ...
                          <p className="text-xs font-bold mt-2 animate-pulse">Crunching bits...</p>
                       </div>
                     ) : (
                  ...
                  {activeSpace.files && activeSpace.files.map((file, i) => (
                    ...
                    >
                       <div className="flex justify-between items-start">
                          <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center border-2 border-transparent">
                             {getFileIcon(file.type)}
                          </div>
           ...
           <div className="relative w-full max-w-2xl bg-white border-4 border-black rounded-3xl overflow-hidden shadow-[0px_0px_0px_4px_rgba(255,255,255,0.2)] animate-in zoom-in-95">
              <div className="h-64 bg-gray-100 border-b-2 border-black flex items-center justify-center relative pattern-dots">
                 {viewingFile.type === 'image' ? (
                    <div className="text-center">
                       ...
                       <p className="font-mono text-xs text-gray-400">Image Preview Placeholder</p>
                    </div>
                 ...
                 <button 
                   ...
                   className="absolute top-4 right-4 w-10 h-10 bg-white border-2 border-black rounded-full flex items-center justify-center hover:bg-gray-100 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none"
                 >
                   <X size={20} />
                 </button>
              </div>
              <div className="p-8">
                 <div className="flex items-start justify-between mb-6">
                    ...
                    <button className="bg-yellow-300 text-black px-4 py-2 rounded-xl border-2 border-black font-bold hover:bg-yellow-400 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-y-0.5 flex items-center gap-2">
                 ...
                 <div className="bg-gray-50 rounded-xl p-4 border-2 border-gray-100">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                       <div>
                          ...
                          <p className="font-bold">{viewingFile.type.toUpperCase()}</p>
                       </div>
                       <div>
                          <p className="text-gray-500">Size</p>
                          <p className="font-bold">{viewingFile.size}</p>
                       </div>
                    </div>
                 </div>
              </div>
              ...
              <>
                <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-yellow-50">
                   ...
                   <div className="space-y-4">
                     <div>
                       ...
                       <input 
                         ...
                         className="w-full px-4 py-3 text-lg font-bold border-2 border-black rounded-xl focus:outline-none focus:ring-4 focus:ring-yellow-300/50 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] transition-shadow"
                         placeholder="e.g. The Bat Cave"
                       />
                     </div>
                     <div>
                       ...
                       <textarea 
                         ...
                         className="w-full px-4 py-3 font-medium border-2 border-black rounded-xl focus:outline-none focus:ring-4 focus:ring-yellow-300/50 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] h-24 resize-none"
                   ...
                   <button 
                    ...
                    className="mt-8 w-full bg-black text-white font-bold text-lg py-4 rounded-xl border-2 border-black hover:bg-gray-900 hover:shadow-[4px_4px_0px_0px_rgba(250,204,21,1)] transition-all disabled:opacity-50 disabled:cursor-not-allowed active:translate-y-[2px] active:shadow-none"
                   >
                     Next Step →
                   </button>
                </div>
                <div className="hidden md:flex w-1/2 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-blue-500 items-center justify-center relative overflow-hidden border-l-4 border-black">
                   ...
                   <div className="relative z-10 text-center p-8">
                      <div className="bg-white border-2 border-black rounded-2xl p-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)] rotate-3 max-w-xs mx-auto">
                        <div className="h-32 bg-gray-100 rounded-xl mb-4 flex items-center justify-center text-4xl">
                           🏰
                        </div>
            ...
            {createStep === 2 && (
              <div className="w-full h-full flex flex-col">
                <div className="p-6 border-b-2 border-black flex items-center bg-pink-50">
                  <button onClick={() => setCreateStep(1)} className="mr-4 p-2 hover:bg-white rounded-lg transition-colors">
                    <ArrowLeft size={24} />
                  </button>
                  <div>
                    <span className="text-xs font-bold uppercase tracking-wider text-pink-600">Step 2/3</span>
                    <h2 className="text-2xl font-black">Choose a Vibe</h2>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 bg-white">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {spaceTemplates.map((t) => (
                      <button 
                        key={t.id}
                        onClick={() => handleCreateConfirm(t)}
                        className="group text-left border-2 border-black rounded-2xl p-4 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 transition-all active:translate-y-0 active:shadow-none bg-white"
                      >
                         <div 
                           className="h-32 rounded-xl mb-4 border-2 border-black flex items-center justify-center text-white"
             ...
             {createStep === 3 && (
              <div className="w-full p-8 md:p-12 flex flex-col items-center justify-center text-center bg-[#FFFDF5]">
                 <div className="w-20 h-20 bg-green-400 rounded-full border-4 border-black flex items-center justify-center mb-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] animate-bounce">
                   <Smile size={40} className="text-black" />
                 </div>
                 <h2 className="text-3xl font-black mb-2">Space Created! 🎉</h2>
                 <p className="text-gray-600 font-medium mb-8 max-w-md">Your space is ready for action. Invite your friends to join the fun!</p>
                 ...
                 <div className="w-full max-w-md bg-white border-2 border-black rounded-xl p-2 flex items-center shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] mb-6">
                    <div className="flex-1 px-4 font-mono text-sm truncate text-gray-600">
                      {createdSpaceLink}
                    </div>
                    <button className="bg-yellow-300 hover:bg-yellow-400 text-black p-2.5 rounded-lg border-2 border-black font-bold transition-colors">
                      <Copy size={18} />
                    </button>
                 </div>

                 <div className="flex gap-4">
                   <button onClick={handleFinalizeCreate} className="px-6 py-3 bg-white border-2 border-black rounded-xl font-bold hover:bg-gray-50 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none">
                     Skip Invite
                   </button>
                   <button onClick={handleFinalizeCreate} className="px-6 py-3 bg-black text-white border-2 border-black rounded-xl font-bold hover:shadow-[4px_4px_0px_0px_rgba(236,72,153,1)] transition-shadow">
                     Go to Space →
                   </button>
                 </div>
              </div>
